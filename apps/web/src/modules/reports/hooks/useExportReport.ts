...
export function downloadStudentReportPdf(
  ...
  return apiClient.download(
    `/reports/students/${studentId}/export${query}`,
    `reporte-${slugify(label)}.pdf`,
  );
}
...
export function downloadBulletinPdf(studentId: string, label: string, academicPeriodId?: string) {
  const query = academicPeriodId ? `?academicPeriodId=${academicPeriodId}` : '';
  return apiClient.download(
    `/reports/students/${studentId}/bulletin/export${query}`,
    `boletin-${slugify(label)}.pdf`,
  );
}
...
export function downloadBulletinCsv(studentId: string, label: string, academicPeriodId?: string) {
  ...
  return apiClient.download(
    `/reports/students/${studentId}/bulletin/export${query}`,
    `boletin-${slugify(label)}.csv`,
  );
}

export function downloadCourseReportCsv(
  courseId: string,
  label: string,
  academicPeriodId?: string,
) {
  const query = academicPeriodId ? `?academicPeriodId=${academicPeriodId}` : '';
  return apiClient.download(
    `/reports/courses/${courseId}/export${query}`,
    `reporte-curso-${slugify(label)}.csv`,
  );
}
