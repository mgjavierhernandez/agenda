...
import { useCreateCourseDirectorAssignment, useUsers } from '../hooks';
...
import type { CreateCourseDirectorAssignmentInput } from '@/api/types';
...
export function CourseDirectorFormPage() {
  const navigate = useNavigate();
  const { hasPermission } = usePermissions();
  const canManage = hasPermission(PERMISSIONS.TEACHER_ASSIGNMENTS_MANAGE);

  const createMutation = useCreateCourseDirectorAssignment();
  ...
  const isLoadingSelectors = isLoadingUsers || isLoadingCourses || isLoadingPeriods;
  ...
  const validate = (): boolean => {
    ...
    if (!directorUserId) {
      newErrors.directorUserId = 'El director es requerido';
    }
    if (!courseId) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: CreateCourseDirectorAssignmentInput = {
      directorUserId,
      courseId,
      academicPeriodId,
      startDate,
    ...
    try {
      ...
      navigate(`/course-directors/${created.id}`);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Nueva asignación de director de grupo"
        description="Asignar un director de grupo a un curso y periodo académico"
      />
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <Button variant="secondary" type="button" onClick={() => navigate('/course-directors')}>
            Cancelar
          </Button>
