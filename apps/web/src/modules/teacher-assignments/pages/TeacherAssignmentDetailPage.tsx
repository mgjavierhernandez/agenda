...
export function TeacherAssignmentDetailPage() {
  ...
  const { data: assignment, isLoading, error } = useTeacherAssignment(id ?? '');
  const deactivateMutation = useDeactivateTeacherAssignment();
  const updateMutation = useUpdateTeacherAssignment();

  const { data: usersData } = useUsers({ limit: 200 });
  const { data: course } = useCourse(assignment?.courseId ?? '');
  const { data: subject } = useSubject(assignment?.subjectId ?? '');
  ...
  const teacherName = usersData?.data.find((u) => u.id === assignment?.teacherUserId);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={`Asignación — ${teacherName ? `${teacherName.firstName} ${teacherName.lastName}` : 'Profesor'}`}
        description={`Curso: ${course?.name ?? '—'} | Asignatura: ${subject?.name ?? '—'}`}
        actions={
        ...
        <Card>
          ...
          <dl className="space-y-3">
            ...
            <div>
              ...
              <dd className="text-gray-900">{teacherName?.email ?? '—'}</dd>
            </div>
            <div>
