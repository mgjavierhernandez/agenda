    ...
    <div className="space-y-6">
      ...
      {canManage && (
        <Card>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
            <Input label="Código" placeholder="A-101" value={code} onChange={(e) => setCode(e.target.value)} required />
            <Input label="Nombre" placeholder="Aula 101" value={name} onChange={(e) => setName(e.target.value)} required />
            <Input label="Capacidad" type="number" min={1} placeholder="30" value={capacity} onChange={(e) => setCapacity(e.target.value)} />
            <div>
              <label htmlFor="roomType" className="block text-sm font-medium text-gray-700 mb-1">Tipo</label>
              <select
                id="roomType"
                value={type}
              ...
              >
                {(Object.keys(CLASSROOM_TYPE_LABELS) as ClassroomType[]).map((t) => (
                  <option key={t} value={t}>{CLASSROOM_TYPE_LABELS[t]}</option>
                ))}
              </select>
            </div>
            <Button type="submit" isLoading={createClassroom.isPending}>Crear</Button>
          </form>
      ...
      <Card>
        <div className="mb-4 max-w-sm">
          <Input label="Buscar" placeholder="Código o nombre…" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        {isLoading ? (
