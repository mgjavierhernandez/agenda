...
function useMembershipDecision(action: 'approve' | 'reject') {
  ...
  return useMutation<UserMembership, Error, { institutionId: string | null | undefined; membershipId: string }>({
    mutationFn: ({ institutionId, membershipId }) =>
      apiClient.post<UserMembership>(`/institutions/${institutionId}/memberships/${membershipId}/${action}`, {}),
    onSuccess: () => {
