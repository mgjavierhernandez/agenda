...
export function useDeleteCommunicationAttachment() {
  ...
  return useMutation<{ success: boolean }, Error, { communicationId: string; attachmentId: string }>({
    mutationFn: ({ communicationId, attachmentId }) =>
      apiClient.delete<{ success: boolean }>(`/communications/${communicationId}/attachments/${attachmentId}`),
