...
import type { CourseStatus, CreateCourseInput, UpdateCourseInput } from '@/api/types';
...
export function CourseFormPage() {
  ...
  const { data: existingCourse, isLoading: isLoadingCourse } = useCourse(id ?? '');
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <PageHeader
        ...
        description={isEditing ? 'Actualizar información del curso' : 'Registrar un nuevo curso'}
      />
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          <div className="flex justify-end gap-3 pt-4">
            <Button
              type="button"
              variant="secondary"
              onClick={() => navigate(isEditing ? `/courses/${id}` : '/courses')}
              disabled={isSubmitting}
            >
              Cancelar
            </Button>
