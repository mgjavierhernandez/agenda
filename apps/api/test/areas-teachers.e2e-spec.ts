...
describe('Areas + Teachers academic relations (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const secondInstitution = await prisma.institution.create({
      data: { name: 'Second School AT', slug: 'second-schoolat-e2e', status: 'ACTIVE' },
    });
    ...
    const secondAdmin = await prisma.user.create({
      data: {
        email: 'admin@second-schoolat-e2e.dev',
    ...
    const secondAdminMem = await prisma.userInstitution.create({
      data: { userId: secondAdmin.id, institutionId: secondInstitutionId, status: 'ACTIVE' },
    });
  ...
  afterAll(async () => {
    if (createdAreaId) {
      await prisma.area.delete({ where: { id: createdAreaId } }).catch(() => {});
    }
  ...
  describe('GET /api/v1/areas', () => {
    ...
    it('should return seeded demo areas for admin with tenant scoping', async () => {
      ...
      for (const area of res.body.data) {
        expect(area.institutionId).toBe(demoInstitutionId);
      }
    });
  });
  ...
  describe('POST /api/v1/areas', () => {
    it('should create an area and scope it to the acting institution', async () => {
      const res = await request(app.getHttpServer())
        ...
        .set('X-Institution-Id', demoInstitutionId)
        .send({
          code: 'ARE-E2E' + Date.now().toString().slice(-6),
          name: 'E2E Area',
          isOfficial: true,
        })
        .expect(201);
    ...
    it('should reject body tampering with a cross-tenant area', async () => {
      await request(app.getHttpServer())
        ...
        .set('X-Institution-Id', demoInstitutionId)
        .send({
          code: 'ARE-T' + Date.now().toString().slice(-6),
          name: 'Tamper',
          isOfficial: true,
          institutionId: secondInstitutionId,
        })
  ...
  describe('Subjects academic fields (areaId/subjectType/level)', () => {
    ...
    it('should reject a subject referencing an area from another institution', async () => {
      const otherArea = await prisma.area.findFirst({
      ...
      if (otherArea) {
        await request(app.getHttpServer())
          ...
          .set('X-Institution-Id', demoInstitutionId)
          .send({
            code: 'SUB-X' + Date.now().toString().slice(-6),
            name: 'X',
            areaId: otherArea.id,
          })
          .expect(404);
  ...
  describe('GET /api/v1/teacher-assignments/teachers', () => {
    it('should list teachers with courses/subjects/students', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/v1/teacher-assignments/teachers')
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(200);
  ...
  describe('GET /api/v1/teacher-assignments/directories', () => {
    it('should list group directors (DIRECTOR_DE_GRUPO role holders)', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/v1/teacher-assignments/directories')
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(200);
      expect(Array.isArray(res.body)).toBe(true);
      const demoTeacher = res.body.find(
        (d: { email: string }) => d.email === 'teacher@demo-school.dev',
      );
      expect(demoTeacher).toBeDefined();
    });
  });
