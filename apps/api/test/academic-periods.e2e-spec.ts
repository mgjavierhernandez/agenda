...
describe('Academic Periods Module (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const secondInstitution = await prisma.institution.create({
      data: { name: 'Second School AP', slug: 'second-schoolap-e2e', status: 'ACTIVE' },
    });
    ...
    const secondTeacher = await prisma.user.create({
      data: {
        email: 'teacher@second-schoolap-e2e.dev',
    ...
    const secondAdmin = await prisma.user.create({
      data: {
        email: 'admin@second-schoolap-e2e.dev',
        passwordHash: VALID_PASSWORD_HASH,
        firstName: 'Second',
        lastName: 'Admin',
        status: 'ACTIVE',
  ...
  describe('RBAC', () => {
    it('should return 403 for teacher without academic-periods:manage', async () => {
      await request(app.getHttpServer())
        ...
        .set('X-Institution-Id', demoInstitutionId)
        .send({ name: 'T', code: 'TF', startDate: '2026-01-01', endDate: '2026-06-30' })
        .expect(403);
    });
    it('should return 403 for parent without academic-periods:manage', async () => {
      await request(app.getHttpServer())
        .patch('/api/v1/academic-periods/00000000-0000-0000-0000-000000000000')
  ...
  describe('POST /api/v1/academic-periods', () => {
    ...
    it('should reject duplicate code within same tenant', async () => {
      await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .send({ name: 'Dup', code: UNIQUE_CODE, startDate: '2026-01-01', endDate: '2026-06-30' })
        .expect(409);
    });
    it('should allow same code in different tenant', async () => {
      const sa = await request(app.getHttpServer())
        .post('/api/v1/auth/login')
        .send({ email: 'admin@second-schoolap-e2e.dev', password: 'Demo1234!' });
      await request(app.getHttpServer())
        .post('/api/v1/academic-periods')
        .set('Authorization', `Bearer ${sa.body.accessToken}`)
        .set('X-Institution-Id', secondInstitutionId)
        .send({ name: 'Cross', code: UNIQUE_CODE, startDate: '2026-01-01', endDate: '2026-06-30' })
        .expect(201);
    });
  ...
  describe('GET /api/v1/academic-periods', () => {
    it('should return paginated list', async () => {
      ...
      for (const ap of res.body.data) {
        expect(ap.institutionId).toBe(demoInstitutionId);
      }
    });
    ...
    it('should apply search filter', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/v1/academic-periods?search=2026')
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(200);
      for (const ap of res.body.data) {
    ...
    it('should return correct pagination meta', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/v1/academic-periods?page=1&limit=1')
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(200);
  ...
  describe('GET /api/v1/academic-periods/:id', () => {
    it('should return academic period by id', async () => {
      const res = await request(app.getHttpServer())
        .get(`/api/v1/academic-periods/${createdAcademicPeriodId}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(200);
    ...
    it('should return 404 for non-existent', async () => {
      await request(app.getHttpServer())
        .get('/api/v1/academic-periods/00000000-0000-0000-0000-000000000000')
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(404);
    });
    it('should return 404 for cross-tenant resource', async () => {
      const other = await prisma.academicPeriod.findFirst({
        where: { institutionId: secondInstitutionId },
      });
      if (other) {
        await request(app.getHttpServer())
          .get(`/api/v1/academic-periods/${other.id}`)
          .set('Authorization', `Bearer ${adminToken}`)
          .set('X-Institution-Id', demoInstitutionId)
          .expect(404);
      }
  ...
  describe('PATCH /api/v1/academic-periods/:id', () => {
    it('should update academic period', async () => {
      const res = await request(app.getHttpServer())
        .patch(`/api/v1/academic-periods/${createdAcademicPeriodId}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
  ...
  describe('PATCH /api/v1/academic-periods/:id/deactivate', () => {
    it('should deactivate academic period', async () => {
      const res = await request(app.getHttpServer())
        .patch(`/api/v1/academic-periods/${createdAcademicPeriodId}/deactivate`)
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(200);
    ...
    it('should return 404 for cross-tenant deactivation', async () => {
      ...
      if (other) {
        await request(app.getHttpServer())
          .patch(`/api/v1/academic-periods/${other.id}/deactivate`)
          .set('Authorization', `Bearer ${adminToken}`)
          .set('X-Institution-Id', demoInstitutionId)
          .expect(404);
      }
  ...
  describe('Tenant Isolation', () => {
    it('should not allow Tenant A to access Tenant B resources', async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .expect(200);
      for (const ap of res.body.data) {
        expect(ap.institutionId).toBe(secondInstitutionId);
      }
    });
  });
  ...
  describe('Duplication Prevention', () => {
    it('should reject duplicate creation', async () => {
      await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .send({ name: 'Dup2', code: UNIQUE_CODE, startDate: '2026-01-01', endDate: '2026-06-30' })
        .expect(409);
    });
  });
  ...
  describe('PROMPT 90: Close academic period (OPEN -> CLOSED)', () => {
    ...
    it('should create an OPEN period for closure tests', async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .send({ name: 'Close Me', code, startDate: '2027-01-01', endDate: '2027-06-30' })
        .expect(201);
    ...
    it('should close an OPEN period as admin -> status CLOSED with authority fields', async () => {
      const res = await request(app.getHttpServer())
        .patch(`/api/v1/academic-periods/${closePeriodId}/close`)
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(200);
    ...
    it('should forbid editing a CLOSED period -> 400', async () => {
      await request(app.getHttpServer())
        .patch(`/api/v1/academic-periods/${closePeriodId}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
    ...
    it('should forbid deactivating a CLOSED period -> 400', async () => {
      await request(app.getHttpServer())
        .patch(`/api/v1/academic-periods/${closePeriodId}/deactivate`)
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(400);
    });
    ...
    it('should return 404 closing a cross-tenant period', async () => {
      ...
      if (other) {
        await request(app.getHttpServer())
          .patch(`/api/v1/academic-periods/${other.id}/close`)
          .set('Authorization', `Bearer ${adminToken}`)
          .set('X-Institution-Id', demoInstitutionId)
          .expect(404);
      }
