...
describe('Guardians Module (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const secondInstitution = await prisma.institution.create({ data: { name: 'Second School GU', slug: 'second-schoolgu-e2e', status: 'ACTIVE' } });
    ...
    for (const perm of allPermissions) { await prisma.rolePermission.create({ data: { roleId: secondTeacherRole.id, permissionId: perm.id } }); }

    const secondAdmin = await prisma.user.create({ data: { email: 'admin@second-schoolgu-e2e.dev', passwordHash: VALID_PASSWORD_HASH, firstName: 'Second', lastName: 'Admin', status: 'ACTIVE' } });
    ...
    const secondTeacher = await prisma.user.create({ data: { email: 'teacher@second-schoolgu-e2e.dev', passwordHash: VALID_PASSWORD_HASH, firstName: 'Second', lastName: 'Teacher', status: 'ACTIVE' } });
    ...
    const secondStudent = await prisma.student.create({ data: { institutionId: secondInstitutionId, firstName: 'Second', lastName: 'Student', documentType: 'DNI', documentNumber: `SG${Date.now().toString().slice(-8)}` } });
    ...
    const testStudent = await prisma.student.create({ data: { institutionId: demoInstitutionId, firstName: 'Guardian', lastName: 'Test', documentType: 'DNI', documentNumber: `GU${Date.now().toString().slice(-8)}` } });
  ...
  afterAll(async () => {
    ...
    await prisma.user.delete({ where: { email: 'teacher@second-schoolgu-e2e.dev' } }).catch(() => {});
  ...
  describe('Authentication', () => {
    it('should return 401 without auth token', async () => {
      await request(app.getHttpServer()).get('/api/v1/guardians/students').set('X-Institution-Id', demoInstitutionId).expect(401);
    });
  });

  describe('RBAC', () => {
    it('should return 403 for student without guardians:manage', async () => {
      const studentToken = await (async () => { const res = await request(app.getHttpServer()).post('/api/v1/auth/login').send({ email: 'student@demo-school.dev', password: 'Demo1234!' }); return res.body.accessToken; })();
      await request(app.getHttpServer()).post(`/api/v1/guardians/students/${testStudentId}`).set('Authorization', `Bearer ${studentToken}`).set('X-Institution-Id', demoInstitutionId).send({ studentId: testStudentId, relationshipType: 'OTHER' }).expect(403);
    });
  });

  describe('POST /api/v1/guardians/students/:studentId', () => {
    it('should link guardian to student as parent', async () => {
      const res = await request(app.getHttpServer()).post(`/api/v1/guardians/students/${testStudentId}`).set('Authorization', `Bearer ${adminToken}`).set('X-Institution-Id', demoInstitutionId).send({ studentId: testStudentId, relationshipType: 'FATHER', isPrimary: true }).expect(201);
      expect(res.body).toHaveProperty('id');
      expect(res.body.relationshipType).toBe('FATHER');
      expect(res.body.isPrimary).toBe(true);
    });
    it('should reject missing required fields', async () => {
      await request(app.getHttpServer()).post(`/api/v1/guardians/students/${testStudentId}`).set('Authorization', `Bearer ${adminToken}`).set('X-Institution-Id', demoInstitutionId).send({}).expect(400);
    });
    it('should reject duplicate guardian-student link', async () => {
      await request(app.getHttpServer()).post(`/api/v1/guardians/students/${testStudentId}`).set('Authorization', `Bearer ${adminToken}`).set('X-Institution-Id', demoInstitutionId).send({ studentId: testStudentId, relationshipType: 'FATHER' }).expect(409);
    });
  });

  describe('GET /api/v1/guardians/students', () => {
    it('should return students linked to guardian (admin)', async () => {
      const res = await request(app.getHttpServer()).get('/api/v1/guardians/students').set('Authorization', `Bearer ${adminToken}`).set('X-Institution-Id', demoInstitutionId).expect(200);
      expect(res.body).toHaveProperty('data');
      expect(Array.isArray(res.body.data)).toBe(true);
    });
    it('should return correct pagination meta', async () => {
      const res = await request(app.getHttpServer()).get('/api/v1/guardians/students?page=1&limit=2').set('Authorization', `Bearer ${adminToken}`).set('X-Institution-Id', demoInstitutionId).expect(200);
  ...
  describe('GET /api/v1/guardians/students/:studentId/guardians', () => {
    it('should return guardians for a student', async () => {
      const res = await request(app.getHttpServer()).get(`/api/v1/guardians/students/${testStudentId}/guardians`).set('Authorization', `Bearer ${adminToken}`).set('X-Institution-Id', demoInstitutionId).expect(200);
  ...
  describe('DELETE /api/v1/guardians/students/:studentId', () => {
    it('should unlink guardian from student', async () => {
      await request(app.getHttpServer()).delete(`/api/v1/guardians/students/${testStudentId}`).set('Authorization', `Bearer ${adminToken}`).set('X-Institution-Id', demoInstitutionId).expect(200);
      const res = await request(app.getHttpServer()).get('/api/v1/guardians/students').set('Authorization', `Bearer ${adminToken}`).set('X-Institution-Id', demoInstitutionId).expect(200);
  ...
  describe('Tenant Isolation', () => {
    it('should not allow Tenant A to access Tenant B resources', async () => {
      const res = await request(app.getHttpServer()).get('/api/v1/guardians/students').set('Authorization', `Bearer ${secondAdminToken}`).set('X-Institution-Id', secondInstitutionId).expect(200);
