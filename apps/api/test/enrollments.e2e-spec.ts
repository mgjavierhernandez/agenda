...
describe('Enrollments Module (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const secondInstitution = await prisma.institution.create({
      data: { name: 'Second School EN', slug: 'second-schoolen-e2e', status: 'ACTIVE' },
    });
    ...
    const secondTeacher = await prisma.user.create({
      data: {
        email: 'teacher@second-schoolen-e2e.dev',
    ...
    const secondAdmin = await prisma.user.create({
      data: {
        email: 'admin@second-schoolen-e2e.dev',
        passwordHash: VALID_PASSWORD_HASH,
        firstName: 'Second',
        lastName: 'Admin',
        status: 'ACTIVE',
    ...
    const ts = await prisma.student.create({
      data: {
        institutionId: demoInstitutionId,
        firstName: 'Enroll',
        lastName: 'Test',
        documentType: 'DNI',
        documentNumber: 'EN' + Date.now().toString().slice(-8),
      },
    });
    testStudentId = ts.id;
    ...
    const sg = await prisma.schoolGrade.create({
      data: {
        institutionId: demoInstitutionId,
        name: 'E2E Grade',
        code: 'EG' + Date.now().toString().slice(-6),
      },
    });
    ...
    const ap = await prisma.academicPeriod.create({
      data: {
        institutionId: demoInstitutionId,
        name: 'E2E Period',
        code: 'EP' + Date.now().toString().slice(-6),
  ...
  describe('RBAC', () => {
    it('should return 403 for teacher without enrollments:manage', async () => {
      await request(app.getHttpServer())
        ...
        .send({
          ...
          schoolGradeId: testSchoolGradeId,
    ...
    it('should return 403 for parent without enrollments:manage', async () => {
      await request(app.getHttpServer())
        .patch('/api/v1/enrollments/00000000-0000-0000-0000-000000000000')
  ...
  describe('GET /api/v1/enrollments', () => {
    it('should return paginated list', async () => {
      ...
      for (const en of res.body.data) {
        expect(en.institutionId).toBe(demoInstitutionId);
      }
    });
    it('should return correct pagination meta', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/v1/enrollments?page=1&limit=1')
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(200);
  ...
  describe('GET /api/v1/enrollments/:id', () => {
    ...
    it('should return 404 for non-existent', async () => {
      await request(app.getHttpServer())
        .get('/api/v1/enrollments/00000000-0000-0000-0000-000000000000')
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(404);
    });
  ...
  describe('PATCH /api/v1/enrollments/:id', () => {
    it('should update enrollment status', async () => {
      const res = await request(app.getHttpServer())
        .patch(`/api/v1/enrollments/${createdEnrollmentId}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
  ...
  describe('PATCH /api/v1/enrollments/:id/deactivate', () => {
    it('should deactivate enrollment', async () => {
      const res = await request(app.getHttpServer())
        .patch(`/api/v1/enrollments/${createdEnrollmentId}/deactivate`)
        .set('Authorization', `Bearer ${adminToken}`)
        .set('X-Institution-Id', demoInstitutionId)
        .expect(200);
      expect(res.body.status).toBe('INACTIVE');
    ...
    it('should return 404 for cross-tenant deactivation', async () => {
      ...
      if (other) {
        await request(app.getHttpServer())
          .patch(`/api/v1/enrollments/${other.id}/deactivate`)
          .set('Authorization', `Bearer ${adminToken}`)
          .set('X-Institution-Id', demoInstitutionId)
          .expect(404);
      }
  ...
  describe('Tenant Isolation', () => {
    it('should not allow Tenant A to access Tenant B resources', async () => {
      const sa = await request(app.getHttpServer())
        .post('/api/v1/auth/login')
        .send({ email: 'admin@second-schoolen-e2e.dev', password: 'Demo1234!' });
      const res = await request(app.getHttpServer())
        .get('/api/v1/enrollments')
        ...
        .expect(200);
      for (const en of res.body.data) {
        expect(en.institutionId).toBe(secondInstitutionId);
      }
    });
  });
