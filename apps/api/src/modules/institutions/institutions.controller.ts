...
} from '../auth/tenant/optional-tenant.guard';
...
export class InstitutionsController {
  ...
  async create(@Request() req: AuthenticatedRequest, @Body() dto: CreateInstitutionDto) {
  ...
  async findAll(@Request() req: AuthenticatedRequest, @Query() query: ListInstitutionsQueryDto) {
  ...
  async deactivate(@Request() req: AuthenticatedRequest, @Param('id', ParseUUIDPipe) id: string) {
    return this.institutionsService.deactivate(
      id,
      req.user.userId,
      req.tenant?.institutionId,
      req.ip,
