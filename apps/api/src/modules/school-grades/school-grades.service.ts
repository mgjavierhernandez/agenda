...
export class SchoolGradesService {
  ...
  async create(
    ...
    await this.auditService.log({
      ...
      newValues: {
        ...
        code: schoolGrade.code,
        sortOrder: schoolGrade.sortOrder,
      },
      ipAddress,
    });
    return schoolGrade;
  ...
  async findAll(
    institutionId: string,
    query: ListSchoolGradesQueryDto,
  ): Promise<{
    data: SchoolGrade[];
  ...
  async update(
    ...
    if (dto.code) {
      const duplicate = await this.prisma.schoolGrade.findFirst({
        where: { institutionId, code: dto.code, id: { not: id } },
      });
      if (duplicate)
        throw new ConflictException(
          'School grade with this code already exists in this institution',
        );
    }
    ...
    await this.auditService.log({
      ...
      institutionId,
      action:
        dto.status && dto.status === SchoolGradeStatus.INACTIVE
          ? 'SCHOOL_GRADE_DEACTIVATED'
          : 'SCHOOL_GRADE_UPDATED',
      entityType: 'SchoolGrade',
      entityId: schoolGrade.id,
      oldValues: {
        name: existing.name,
