...
export class TaskAssignmentsController {
  ...
  async create(@Request() req: AuthenticatedRequest, @Body() dto: CreateTaskAssignmentDto) {
  ...
  async findAll(@Request() req: AuthenticatedRequest, @Query() query: ListTaskAssignmentsQueryDto) {
    return this.taskAssignmentsService.findAll(req.tenant!.institutionId, query, req.user.userId);
  }
  ...
  async findOne(@Request() req: AuthenticatedRequest, @Param('id', ParseUUIDPipe) id: string) {
    return this.taskAssignmentsService.findOne(req.tenant!.institutionId, id, req.user.userId);
  }
  ...
  @ApiResponse({ status: 404, description: 'Task assignment not found' })
  async markOpened(@Request() req: AuthenticatedRequest, @Param('id', ParseUUIDPipe) id: string) {
